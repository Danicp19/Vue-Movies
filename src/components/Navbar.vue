<script setup lang="ts">

</script>

<template>
  <div
    class="sticky top-0 z-30 flex h-16 w-full justify-center text-base-content opacity-90 backdrop-blur transition-all duration-100"
  >
    <nav class="navbar w-full">
      <div class="flex flex-1 md:gap-1 lg:gap-2">

   
        <router-link to="/" 
     
        
        class="px-2">
          <div
            class="inline-flex text-lg text-primary transition-all duration-200 md:text-3xl"
          >
            <span class="text-blue-400">V<span class="lowercase">ue</span></span>
            <span class="text-gray-500">M<span class="lowercase">ovies</span></span>
          </div>
        </router-link>
        <router-link to="/" class="px-2">
          <div
            class="inline-flex text-lg text-primary transition-all duration-200 md:text-3xl"
          >
            <span class="text-blue-400">Home</span>
          </div>
        </router-link>
        <router-link to="/favorites/" class="px-2">
          <div
            class="inline-flex text-lg text-primary transition-all duration-200 md:text-3xl"
          >
         
              <span class="text-blue-400">Favorites</span>
            
          </div>
        </router-link>
      </div>
    </nav>
  </div>
</template>

<style scoped></style>
